...
L=10; % number of channel paths
sigma_2=1; % noise power
PP=-5; % transmit power 
I_max_l=1:1:25; % iteration number of WMMSE
realization=1000;
for reali=1:realization
    ...
    sumrate1=0; 
        for k=1:K 
            ...
            sumrate1=sumrate1+ log2(det(eye(Nr)+(H(:,:,k)*F(:,(k-1)*D+1:k*D)*F(:,(k-1)*D+1:k*D)'*H(:,:,k)'...
                )*inv(Interference+1/P*trace(F'*F)*eye(Nr))));
        end 
        sumrate_WMMSE(reali,I)=sumrate1;
        end
        F=sqrt(P/trace(F*F'))*F;
        ...
        %% F-WMMSE
        Grx=4; Grz=Grx; Gr=Grx*Grz;
        G_R=zeros(L,Gr,K);
        Gtx=8; Gtz=Gtx; Gt=Gtx*Gtz;
        G_T=zeros(L*K,Gt);
    ...
    z_t_ind  = mod(Lambda_t - 1, sqrt(Gt)) + 1;
        ...
        sumrate_FWMMSE(reali,I)=sumrate;
        end
        ...
        %     B(:,:,k)*
        % end
        % obj(reali,II)=trace(B(:,:,k))
 
end
S_MMSE=abs(mean(sumrate_MMSE))
S_WMMSE=abs(mean(sumrate_WMMSE))
S_FWMMSE=abs(mean(sumrate_FWMMSE))
plot(I_max_l,abs(mean(sumrate_MMSE)))
hold on
plot(I_max_l,abs(mean(sumrate_WMMSE)))
hold on
plot(I_max_l,abs(mean(sumrate_FWMMSE)))
